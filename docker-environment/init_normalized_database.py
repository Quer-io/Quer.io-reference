...
try:
    postgres_conn = psycopg2.connect(host="postgres", port="5432", user="queriouser",
                            password="pass1", dbname='postgres')
    postgres_conn.autocommit = True
    postgres_cursor = postgres_conn.cursor()
    postgres_cursor.execute('DROP DATABASE IF EXISTS normaldb;')
    postgres_cursor.execute('CREATE DATABASE normaldb;')
